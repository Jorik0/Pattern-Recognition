...
function b = im_featuresFast(a,gray,features)
...
elseif isa(a,'prdataset')
   ...
   grayIm = getimage(grayImages,1);
   [props,featlab] = getlabfromregionprops(im,grayIm,features);
   % RD 27Nov13, compute dat size before
   ftrial = getfeatfromregionprops(im,grayIm,props);
   out = zeros(size(a,1),numel(ftrial));
   % Run over all images, and compute the features:
   for n = 1:size(a,1)
     out(n,:) = getfeatfromregionprops(getimage(images,n),getimage(grayImages,n),props);
   end 
   % store in a dataset:
...
function im = getimage(images,index)
if isa(images,'cell')
    im = images{index};
else
    im = squeeze(images(:,:,:,index));
end
return
